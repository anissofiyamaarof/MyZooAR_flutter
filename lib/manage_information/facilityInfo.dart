...
class FacilityInfo extends StatefulWidget {
  @override
  _FacilityInfoState createState() => _FacilityInfoState();
}
...
class _FacilityInfoState extends State<FacilityInfo> {
  ...
  Future<void> _loadData(String docID) async {
    try {
      String? facilityDesc = await firestoreService.getFacilityDescription(docID);
      List<String> imageUrls = await firestoreService.getFacilityImages(docID);
      if (facilityDesc != null) {
        textController.text = facilityDesc;
      } else {
  ...
  Future<void> _saveData() async {
    ...
    try {
      if (textController.text.isNotEmpty && _images.isNotEmpty) {
        await firestoreService.addUpdateFacilityDescriptionImage(docID!, textController.text, _images);
      }
      else if (_images.isNotEmpty) {
        await firestoreService.addUpdateFacilityImages(docID!, _images);
      }
      else if (textController.text.isNotEmpty) {
        await firestoreService.addUpdateFacilityDescription(docID!, textController.text);
      } else {
            ...
            children: [
              Expanded(
                child: StreamBuilder<DocumentSnapshot>(
                  stream: firestoreService.getFacilityIndividual(docID!),
