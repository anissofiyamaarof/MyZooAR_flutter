...
class _AnimalInfoState extends State<AnimalInfo> {
  ...
  Future<void> _loadData(String docID) async {
    try {
      String? animalDesc = await firestoreService.getAnimalDescription(docID);
      List<String> imageUrls = await firestoreService.getAnimalImages(docID);
      if (animalDesc != null) {
        textController.text = animalDesc;
      } else {
  ...
  Future<void> _saveData() async {
    ...
    try {
      if (textController.text.isNotEmpty && _images.isNotEmpty) {
        await firestoreService.addUpdateAnimalDescriptionImage(docID!, textController.text, _images);
      } else if (_images.isNotEmpty) {
        await firestoreService.addUpdateAnimalImages(docID!, _images);
      } else if (textController.text.isNotEmpty) {
        await firestoreService.addUpdateAnimalDescription(docID!, textController.text);
      } else {
            ...
            children: [
              Expanded(
                child: StreamBuilder<DocumentSnapshot>(
                  stream: firestoreService.getAnimalIndividual(docID!),
